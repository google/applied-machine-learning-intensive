...
In order to exit command you can type `exit()` or use the `Ctrl+d` key combination.
-->
...
Notice that we don't see the value of the addition. When running commands using a file of commands you have to explicitly print output.
-->
...
<!--
Another way to run Python is in an interactive notebook. Jupyter is a popular type of notebook that combines rich textual documentation and Python code. We will be using notebooks throughout this course. We'll be using a serivce called Colab, which hosts and runs Jupyter notebooks in the cloud.
-->
...
In the third we import the `mean_squared_error` function from the `sklearn.metrics` library. This make only this function available to us. We also don't have to use a prefix.
-->
...
"I'm a pythoista"            # double-quoted string
...
The second example also uses single quotese, but since it has a single quote in it that quote has to be escaped. The backslash is not part of the string, just the single quote in "I'm". Any ideas how you get a backslash in a string?
...
The final two examples are "triple-quoted" strings. These string allow you to type newlines in strings. Typically these types of strings are used for documentation.
-->
...
In the second you can see a list create using the `list()` class.

In the thired example we create a list of integers.

In the fourth example we create a list containing an integer, float, string, and bool. Lists can contains any type of value. This is different than some languages that require lists to all be of the same type.
...
<!--
Python also supports dictinaries. A dictionary is a bundle of key value pairs. These are sometimes called maps in other languages.

In the first two examples you can see two ways to create an empty dictionary. The `dict()` method also can populate a dictionary in intersting ways that you might want to explore.

The third example shows a basic pre-populated dictionary. You can see that keys and values can be different data types. The only real constraint is that keys must be immutable. This means that integers, floats, strings, and tuples can all be used as keys. Dictionaries and lists are not allowed. Also, if a tuple contains an immutable data type is cannot be a key.
-->
...
Finally we subtract `a` from `b`. Notice that in every case that we mixed float and integer math the result was a float.
-->
...
print(a[1:4])     # 2nd-3rd character of string
...
Strings consist of characters. In our second print statement we print the second character of the string `a`. Counting starts at 1. In the thrid print statement we print a slice of a string. We start at the second character (index 1) and end at index 4. Notice that the chacter in index 1 is included in the output, but the character at index 4 is not. Slicing always includes the value at the first index and excludes the final index.

In our fouth example we call `len()` on the string variable `a`. Notice that this is not a method. It is a Python built-in function. Python is a little inconsistent here. There are reasons for this, but they are beyond this scope of this lab.

Finally we use the `+` operator to combine two strings.

There is so much more that you can learn about strings. We encourage you to look at the [offical documentation](https://docs.python.org/3/library/string.html) to familarize yourself with strings and string methods.
-->
...
<!--
In this example we create a list variable. Notice that you can index a list similar to how you can index a string.
...
To index sublists you can stack indexes as we did in the final example. The innter list is at index 4. Within the inner list the string "more" is at index 1.
-->
...
As you in see, dictionaries are modifiable.
...
`if` is the only required part of this series of blocks. There can only be one `if`.

`elif` is optional and there can be as many `elif` cases that you want. `elif` is short for "else if". `elif` statements are executed in order. The first case to be true has it's block execused and none of the rest are considered.

`else` has it's block ran if none of the cases before are true. There can be zero or one `else`. It always has to come last.
...
```
<!--
Here we can see two different ways to use a for loop. If we put a list or tuple right of `in`, we get each item in the list in turn. We can also use the `range()` function to get the indexes for each element in turn. We can then use those indexes to pull items out of the list for printing.
-->
...
Warning! Since while loops aren't iterating over iterables but instead are waiting on a boolean condition to be False, it is possible to create infinite loops.
-->
...
<!--
When a value is not known in Python we use the `None` value. `None` is roughly equivalent to `null` or `nil` that you might find in some other langages.

One key thing to know about Python's `None` is that `None == None`. This is different than some languages that consider null values to not equal is other.
